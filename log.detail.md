...
## Session 7.2 — M7 改进：否定检测 + getLocs 鲁棒性 (2026-02-25)

### 问题背景

用 mistral/deepseek 测试时发现两类问题：
1. 文本中有多空格（`"blood  pressure"`），LLM 返回单空格关键词 → `getLocs` 正则不匹配
2. 否定语境（`"denies headache"`, `"he doesn't have fever"`）→ LLM 小模型不可靠，会漏标 negated

### 改动

**`parsers/ann-parser.ts` — `getLocs`**
- 转义特殊正则字符（防 keyword 含 `(`, `.` 崩溃）
- 连续空格 → `\s+`：`"blood pressure"` 能匹配 `"blood  pressure"`
- 用 `m.index + m[0].length` 替代 `regex.lastIndex`（更明确）

**`utils/ollama-client.ts`**
- 去掉 `negated` 字段（改为客户端规则处理）
- JSON 解析前先剥 markdown 代码块（deepseek 等模型会包 ` ```json ``` `），再 fallback 到正则抠取
- Prompt Rules 加 `ONLY use these exact tag names: ${tagList}`（防止模型用 DISORDER 代替 DISEASE）
- `temperature: 0`（待加，排除随机性）

**`utils/auto-annotate.ts` — `isNegatedByContext`**
- 前向窗口（60 字符）：查 `denies/no /doesn't/negative for...`
- 后向窗口（30 字符）：查 `absent/not found/: none...`（处理 `"Fever: absent"` 后置否定）
- Scope breaker：句号 `.!?` 和转折词 `but/however/...` 截断窗口，防跨句误杀

### 设计讨论

- **多次出现同一关键词**（`"his son had fever, he has fever"`）：采用方案 A（标注所有出现），理由是标注辅助工具，假阳性用户可手动删，假阴性更难发现
- **LLM negated 字段**：去掉，改为纯客户端窗口检测。LLM 只负责 `{keyword, tag}`，negated 判断由确定性规则完成
- **Two-Way 算法**：讨论了 JS `indexOf` 的底层实现（V8 用 Two-Way，O(n) 时间 O(1) 空间）
- **NegEx**：当前实现是 NegEx 简化版，复杂嵌套从句（`"the fact that he have fever"`）靠窗口+scope breaker 偶然处理对，完整覆盖需要依存句法树

### 测试

| 文件 | 旧 | 新 |
|------|----|----|
| `ann-parser.test.ts` | 6 | 9 |
| `auto-annotate.test.ts` | 6 | 21 |
| 总计 | 27 | 45 |

新增测试覆盖：getLocs 多空格/特殊字符、前置否定 4 种模式、后置否定 2 种模式、scope breaker、窗口边界、集成测试

### 调试记录

| 问题 | 原因 | 修复 |
|------|------|------|
| `chest pain` 被标注（否定未过滤） | mistral 没识别 "denies" 为否定词 | 加客户端前向窗口，"denies" 在词表里 |
| `nausea` 被误杀 | "doesn't have fever. Reports nausea" — 跨句，"doesn't" 在 60 字符窗口内 | `.!?` 加入 scope breaker，句号截断窗口 |
| deepseek: `Failed to parse LLM response as JSON` | deepseek 不遵守 `format: 'json'`，用 markdown 代码块包裹输出 | 解析前 strip ` ```json ``` `，fallback 到正则抠取 `{...}` |
| deepseek: `hypertension`/`diabetes` 未标注 | 模型返回 `"DISORDER"` 而非 `"DISEASE"`，dtd 里找不到 → 过滤掉 | Prompt 加 `ONLY use these exact tag names: ${tagList}` |
| mistral 小模型输出不稳定，有时返回 `"tag": "N/A"` | temperature=0.8 默认有随机性，`"N/A"` 被过滤掉但结果不可重复 | 待加 `options: { temperature: 0 }` |

### 验证

- ✅ `npm run build` 零错误
- ✅ `npm test` 45 测试全部通过
- ✅ 浏览器测试（deepseek-v3.1）：否定词过滤正常，多空格匹配正常

### 文件变更

| 文件 | 操作 |
|------|------|
| `parsers/ann-parser.ts` | 修改：getLocs 鲁棒性 |
| `parsers/__tests__/ann-parser.test.ts` | 修改：+3 测试 |
| `utils/ollama-client.ts` | 修改：markdown 剥离 + prompt 强化 |
| `utils/auto-annotate.ts` | 修改：双向否定窗口，去掉 negated 字段 |
| `utils/__tests__/auto-annotate.test.ts` | 修改：+15 测试 |
...
## Session 8.0 — 仓库架构整理 + 双仓库推送 (2026-03-04)
...
重构项目（M1-M7）已全部完成，需要将成品 `MedGenie-React/` 推送到实验室公开仓库 `PittNAIL/MedGenie`，同时保留开发仓库的完整历史。

### 1. 外层仓库清理（jay2zxy/MedTator_React）

**移除本地开发文档的 git 跟踪**：

```bash
# 取消跟踪（文件保留在磁盘，git 不再管理）
...
learn.md
reload.md
quantum_test/quantum.md
```

> 关键概念：`.gitignore` 只对未跟踪文件生效。已跟踪的文件必须先 `git rm --cached` 取消跟踪，再加 `.gitignore` 防止以后误添加。

**删除废弃分支和远程**：
```bash
git branch -D release-m6        # 废弃的旧仓库分支
git remote remove pittmail       # 废弃的远程连接（只断开本地链接，不删 GitHub 仓库）
```

**提交并推送**：f69e257 → `origin/jay-dev`

### 2. 成品仓库创建（PittNAIL/MedGenie）
...
PittNAIL/MedGenie 远程已有 `master` 分支（含旧内容），新 `git init` 的仓库没有共同祖先，GitHub 提示 "entirely different commit histories"，无法正常 merge。
...
这将 `master` 的历史替换为 `jay-dev-release` 的历史，之后两个分支共享同一祖先，可正常 merge/PR。

### 3. 双仓库架构

```
MedGenie/                          ← 外层（开发仓库）
├── .git/                          ← remote: jay2zxy/MedTator_React.git
├── MedGenie-React/                ← 内层（成品仓库）
│   ├── .git/                      ← remote: PittNAIL/MedGenie.git
│   └── src/...
├── docs/
├── sample/
└── ...
...
1. **git 就近查找 `.git`**：在哪个目录执行 git 命令，就读哪个 `.git` 的配置
2. **两个 remote 都叫 `origin`**：不冲突，因为属于不同 `.git`
3. **外层 git 把 `MedGenie-React/` 当普通文件夹跟踪**（非 submodule），两边都能正常上传
4. **`.gitignore` ≠ 删除**：只是 git 忽略，不影响本地文件使用和调试
5. **同一仓库支持多 remote**：`git remote add <name> <url>`，但适合推同一套文件；推不同范围的文件用独立 `.git` 更合适
...
## 2026-03-11 - GitHub Release: AppImage 上传问题

### 问题

在 GitHub Release v1.0.0 页面通过 Web UI 上传 Linux AppImage 文件时，.exe 和 .dmg 正常上传成功，但 AppImage 报错：

> Something went really wrong, and we can't process that file. Try another file.

尝试将 AppImage 压缩为 .zip 上传，同样失败。

### 排查

```bash
$ ls -lh MedGenie-1.0.0.AppImage
-rw-r--r-- 1 del 197609 112M Mar  9 23:22 MedGenie-1.0.0.AppImage

$ file MedGenie-1.0.0.AppImage
ELF 64-bit LSB executable, x86-64, version 1 (SYSV), dynamically linked, ...
```

文件完整，112MB，ELF 格式正确。**结论：GitHub Web UI 对 ELF 二进制文件处理有 bug**，非文件本身问题。
...
**登录**（Git Bash MinTTY 不支持 `gh` 交互式提示）：
```bash
# 方案1：Git Bash 加参数绕过交互
gh auth login --web -p https

# 方案2：在 PowerShell/CMD 中运行（原生支持交互）
gh auth login
```
...
**上传**：
```powershell
# PowerShell（Claude Code 的 bash 环境有 TLS 错误，PowerShell 直连正常）
gh release upload v1.0.0 "C:\Users\del\Desktop\MedGenie-1.0.0.AppImage" --repo PittNAIL/MedGenie
# Successfully uploaded 1 asset to v1.0.0
```

Claude Code bash 环境的 TLS 错误（代理/网络导致）：
```
Post "https://uploads.github.com/...": remote error: tls: bad record MAC
```
...
| MedGenie-1.0.0-arm64.dmg | 107.22 MB | macOS (ARM) |
| MedGenie-1.0.0.AppImage | 112 MB | Linux (x86_64) |

### 经验总结

- GitHub Web UI 上传大型 ELF 二进制文件可能失败，用 `gh release upload` CLI 更可靠
- Git Bash (MinTTY) 不兼容 `gh` 的交互式提示，需要 `--web -p https` 参数或改用 PowerShell
- 代理环境下 `gh` 上传大文件可能出现 TLS 错误，关掉代理或换终端解决
